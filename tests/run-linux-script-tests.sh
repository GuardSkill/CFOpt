...
test_linux_country_speed_floor_defaults_and_parser() {
  (
    ...
    [[ "$COUNTRY_MIN_SPEED_MB_PER_SEC" == "JP=10,US=2,KR=3,HK=2,DE=5,GB=3,SG=5" ]] || fail "unexpected country floors"
  ...
  for required_text in 'JP=10,US=2,KR=3,HK=2,DE=5,GB=3,SG=5' 'COUNTRY_MIN_SPEED_MB_PER_SEC' 'CountryMinSpeedMBPerSec' '13.1MB/s'; do
...
china_media = root / "rules" / "ChinaMediaDirect.list"
bilibili = root / "rules" / "Bilibili.list"
...
for path in [full, lite, cmliussss]:
    ...
    media_rule = "ruleset=Direct,https://raw.githubusercontent.com/GuardSkill/CFOpt/main/rules/ChinaMediaDirect.list"
    bili_rule = "ruleset=Direct,https://raw.githubusercontent.com/GuardSkill/CFOpt/main/rules/Bilibili.list"
    if media_rule not in content or bili_rule not in content:
...
for rule_file in [china_media, bilibili]:
    rules = [line.strip() for line in text(rule_file).splitlines() if line.strip() and not line.startswith("#")]
    if len(rules) != len(set(rules)):
        raise SystemExit(f"{rule_file}: duplicate rules are not allowed")
    invalid = [rule for rule in rules if not rule.startswith(("DOMAIN,", "DOMAIN-SUFFIX,"))]
    if invalid:
        raise SystemExit(f"{rule_file}: unsupported router domain rules: {invalid}")
...
missing = required_direct_domains - media_rules
if missing:
    raise SystemExit(f"{china_media}: missing required media/large-traffic Direct domains: {sorted(missing)}")

required_bilibili_domains = {
    "DOMAIN-SUFFIX,bilibili.tv",
    "DOMAIN-SUFFIX,bilivideo.com",
    "DOMAIN-SUFFIX,hdslb.com",
    "DOMAIN-SUFFIX,biliimg.com",
}
bilibili_rules = set(line.strip() for line in text(bilibili).splitlines())
missing = required_bilibili_domains - bilibili_rules
if missing:
    raise SystemExit(f"{bilibili}: missing Bilibili TV/CDN Direct domains: {sorted(missing)}")
PY
}

test_tracked_csv_node_labels_are_ascii_safe() {
  for csv in "$ROOT_DIR/CMCC_BJ.csv" "$ROOT_DIR/CTC_CD.csv"; do
