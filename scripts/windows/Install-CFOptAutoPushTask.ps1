...
$argument = "-NoProfile -ExecutionPolicy Bypass -File `"$ScriptPath`" -WorkDir `"$WorkDir`" -CfstPath `"$CfstPath`" -FocusCountries `"SG,HK,TW,JP,KR,DE,GB`" -IpZipCountrySampleMultipliers `"KR=2,US=0.5`""
...
Register-ScheduledTask `
    ...
    -Principal $principal `
    -Settings $settings `
    -Description "CFOpt daily rolling retest at ${DailyAt}: retest previous CSV nodes, replace weak rows, and upload CloudflareSpeedTest CSV." | Out-Null

Write-Host "Scheduled task installed: $TaskName"
Write-Host "It will run daily at $DailyAt and at startup after $StartupDelayMinutes minute(s)."
