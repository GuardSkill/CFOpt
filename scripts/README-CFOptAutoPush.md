...
2. Extract the folder matching the configured port, such as `443`
3. Merge selected group files such as `HK.txt`, `KR.txt`, `SG.txt`
...
`ip.zip` contains folders named by port. The scripts read only the folder matching the configured port.

- `Port=443` / `PORT=443`: read `443`, do not pass `-tp`, use cfst default 443
- `Port=8443` / `PORT=8443`: read `8443`, pass `-tp 8443`
